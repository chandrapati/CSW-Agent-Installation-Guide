...
|---|---|---|
| Packer build can't access blob storage | Builder VM missing RBAC | Assign managed identity to builder; or SSH-copy package |
| Image version conflict | Duplicate `image_version` | Auto-bump version from CI build number |
| First-boot script never runs | Deprovision didn't reset cloud-init | Ensure `waagent -deprovision+user` was last build step |
| VM registers as image-build host | `csw-agent` not masked before deprovision | Mask `csw-agent` during build (Pattern X) |
| Gallery replication slow | Cross-region async copy | Set all consumer regions in template |
| Clones *Not Active* | First-boot missing `user.cfg` | First-boot must write activation key before agent start |
