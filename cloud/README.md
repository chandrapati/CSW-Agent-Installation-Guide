...
### Cloud-provider installer mirror

For cloud first-boot patterns, mirror the **CSW-generated installer
script** into private cloud storage. Treat that script as a secret:
it contains cluster / activation material generated by your CSW UI.
Do not split the installer into ad hoc `ca.pem`,
`/etc/tetration/sensor.conf`, or package-only flows unless Cisco
TAC has confirmed that pattern for your release.
